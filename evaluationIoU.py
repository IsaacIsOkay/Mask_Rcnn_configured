	...
	if(all_nums[i] == 1):
		...
		for a in range(0, 512):
			for b in range(0,512):
				...
				if(label[a][b] == 1):
					groundPix += 1
				if(phased_mask[a][b] == label[a][b] and phased_mask[a][b] == 1):
					samePix += 1
		print("predPix:%d"%predictionPix,"groundPix:%d"%groundPix, "samePix:%d"%samePix)
		accuracy = (float)(correct/total)
		print("id: %d"%image_id,"accuracy: %f"%accuracy)
		imageCount += 1
		if(groundPix > 0):
			iou = (float)(samePix/(groundPix + predictionPix - samePix))
			print("IoU: %f"%iou)
			iouSum += iou
			iouImgs += 1
	
avgAccuracy = (float)(accSum/imageCount)
avgIou = (float)(iouSum/iouImgs)

print(avgAccuracy, "avg IoU:", iouImgs)
